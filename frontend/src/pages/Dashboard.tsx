...
// ── Palette (from the TrackFinance design mockup; self-contained) ──────────
const C = {
  paper: '#FAFAF7',
  card: '#FFFFFF',
  ink: '#1A1815',
  ...
  return (
    ...
    >
      <div
        ...
        style={{ width: 44, height: 44, borderRadius: 11, border: `1px solid ${C.border}`, color: C.faint }}
      >
...
export default function Dashboard() {
  ...
  const buildSeries = (offsetWindows = 0) => {
    ...
    if (range === '7d') {
      ...
      for (let i = span - 1; i >= 0; i--) {
        ...
        d.setDate(today.getDate() - i - offsetWindows * span);
        buckets.push({
      ...
      windowStart.setDate(today.getDate() - (span - 1) - offsetWindows * span);
      transactions.forEach((t) => {
        const td = startOfDay(new Date(t.date));
        const idx = Math.round((td.getTime() - windowStart.getTime()) / 86400000);
        if (idx >= 0 && idx < span) {
      ...
      const weeks = 4;
      for (let w = 0; w < weeks; w++) buckets.push({ label: `Wk ${w + 1}`, income: 0, expense: 0 });
      const windowStart = new Date(today);
      windowStart.setDate(today.getDate() - (weeks * 7 - 1) - offsetWindows * weeks * 7);
      transactions.forEach((t) => {
        const td = startOfDay(new Date(t.date));
        const days = Math.round((td.getTime() - windowStart.getTime()) / 86400000);
        const wIdx = Math.floor(days / 7);
        if (wIdx >= 0 && wIdx < weeks) {
          if (t.type === 'income') buckets[wIdx].income += t.amount;
          else if (t.type === 'expense') buckets[wIdx].expense += t.amount;
        }
      });
    }
  ...
  const cur = buildSeries(0);
  const prev = buildSeries(1);
  const net = cur.income - cur.expense;
  const hasData = cur.income > 0 || cur.expense > 0;

  // Cumulative net across the window → hero sparkline.
  let run = 0;
  ...
  // Real period-over-period deltas (null when no prior baseline → badge hidden).
  ...
  const windowStart = (() => {
    const today = startOfDay(new Date());
    const d = new Date(today);
    d.setDate(today.getDate() - (range === '7d' ? 6 : 27));
    return d;
  })();
  ...
  if (loading) {
    return (
      <div className="flex flex-1 items-center justify-center" style={{ background: C.paper, fontFamily: FONT }}>
        <div className="text-center">
          <div
            className="mx-auto h-10 w-10 animate-spin rounded-full"
            style={{ border: `2px solid ${C.border}`, borderBottomColor: C.accent }}
          ></div>
          <p className="mt-4" style={{ fontSize: 13, color: C.muted }}>
            Loading dashboard…
          </p>
        </div>
      </div>
  ...
  const eyebrow = (text: string) => <div style={mono}>{text}</div>;
    ...
    delta === null ? null : (
      <span
        style={{
          ...
          padding: '3px 8px',
          borderRadius: 6,
          color: good ? C.incomeText : C.over,
          background: good ? C.incomeSoft : 'rgba(196,69,69,0.12)',
        }}
      >
        {delta >= 0 ? '+' : '−'}
  ...
  return (
    <div className="flex-1 overflow-y-auto" style={{ background: C.paper, fontFamily: FONT, color: C.ink }}>
      <style>{`@media (max-width: 1024px){.dash-row{grid-template-columns:minmax(0,1fr) !important;}}`}</style>
      {/* TOP BAR */}
      <header
        className="flex items-center gap-5"
        style={{ height: 64, padding: '0 28px', background: C.card, borderBottom: `0.5px solid ${C.border}` }}
      >
        <div className="flex flex-col" style={{ gap: 1 }}>
          <div style={{ fontSize: 14.5, fontWeight: 600, letterSpacing: '-0.01em' }}>Dashboard</div>
          <div style={{ ...mono, fontSize: 10 }}>Overview</div>
        </div>
        <div className="relative mx-auto" style={{ flex: 1, maxWidth: 520 }}>
          <Search
            size={16}
            strokeWidth={1.5}
        ...
        <div
          ...
          style={{ gap: 9, padding: '4px 10px 4px 4px', border: `1px solid ${C.border}`, borderRadius: 999 }}
        >
          <div
            className="flex items-center justify-center"
            style={{ width: 28, height: 28, borderRadius: '50%', background: C.ink, color: C.paper, fontSize: 11, fontWeight: 600 }}
          >
            {initials}
          </div>
      ...
      {/* MAIN */}
      <main style={{ padding: '30px 36px 48px' }}>
        <div style={{ maxWidth: 1320, margin: '0 auto' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0,1fr) 352px', gap: 24, marginTop: 26 }} className="dash-row">
            {/* Net balance hero */}
                ...
                <div>
                  ...
                  <div className="flex items-center" style={{ marginTop: 16, gap: 14 }}>
                    <span style={{ fontSize: 13, color: C.muted }}>
                      Level <b style={{ color: C.ink, fontWeight: 600 }}>{level}</b>
                    </span>
                  ...
                  {hasData && (
                    <span
                      ...
                      style={{
                        ...
                        textTransform: 'uppercase',
                        fontWeight: 500,
                        color: net >= 0 ? C.incomeText : C.over,
                        background: net >= 0 ? 'rgba(91,163,114,0.14)' : 'rgba(196,69,69,0.12)',
                        border: `1px solid ${net >= 0 ? 'rgba(91,163,114,0.32)' : 'rgba(196,69,69,0.3)'}`,
                        borderRadius: 6,
                        padding: '4px 9px',
                      }}
                    >
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0,1fr) 352px', gap: 24, marginTop: 24 }} className="dash-row">
            {/* Balance overview */}
                  ...
                  {(['7d', '30d'] as const).map((r) => (
                    <button
                      ...
                      style={{
                        ...
                        color: range === r ? C.ink : C.muted,
                        boxShadow: range === r ? '0 1px 2px rgba(26,24,21,0.10)' : 'none',
                      }}
                    >
                      {r}
                    </button>
              ...
              {categories.length > 0 ? (
                <>
                  <div style={{ position: 'relative', width: 188, height: 188, margin: '18px auto 4px' }}>
                    <ResponsiveContainer width="100%" height="100%">
                      <PieChart>
                        <Pie data={categories} dataKey="amount" nameKey="name" innerRadius={62} outerRadius={80} paddingAngle={2} stroke="none" startAngle={90} endAngle={-270}>
                          {categories.map((c2) => (
                    ...
                    <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', pointerEvents: 'none' }}>
                      ...
                      <div style={{ fontSize: 34, fontWeight: 600, letterSpacing: '-0.025em', fontVariantNumeric: 'tabular-nums', lineHeight: 1.1 }}>{fmt(catTotal)}</div>
                  ...
                  <div className="flex flex-col" style={{ gap: 1, marginTop: 14 }}>
                    {categories.map((c2, i) => (
                      <div
                        key={c2.name}
                        className="flex items-center"
                        style={{ gap: 10, padding: '7px 0', borderBottom: i < categories.length - 1 ? `0.5px solid ${C.divider}` : 'none' }}
                      >
